...
export const Card: React.FC<CardProps> = ({ card, faceUp = true, className = '' }) => {
  if (!faceUp || !card) {
    return (
      <div data-testid="card-face-down" className={`w-14 h-20 sm:w-16 sm:h-24 rounded-lg bg-gradient-to-br from-blue-700 to-blue-900 border border-white/30 shadow-md flex items-center justify-center ${className}`}>
  ...
  return (
    <div data-testid="card-face-up" className={`w-14 h-20 sm:w-16 sm:h-24 rounded-lg bg-white shadow-md flex flex-col justify-between p-1.5 ${color} ${className}`}>
