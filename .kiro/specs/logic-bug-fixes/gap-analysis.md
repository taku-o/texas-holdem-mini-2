...
| 現在の実装 | `while (next !== currentIndex && ...)` — 全員が条件に合致しない場合`next`が`currentIndex`に戻るまでループ。ただし全員がフォールド/チップ0の場合、`currentIndex`自体もスキップ条件に合致するときに自分のインデックス（非アクティブなプレイヤー）をそのまま返してしまう。これは無限ループではないが、非アクティブなプレイヤーのインデックスを返すという不正な結果になる | **ギャップ: Missing** — ループ回数上限がなく、全員非アクティブ時に不正な値を返す。-1を返して「アクティブプレイヤーなし」を明示するロジックがない |
...
### 推奨: オプション A + BUG-G4のみオプション B

理由: BUG-G7, BUG-E1, BUG-G5は純粋関数への小規模修正のためオプションAが適切。BUG-G4はオプションB（`dealCommunityCards`純粋関数の抽出）を採用する。フェーズ3で確立した`gameLogic.ts`への抽出パターンに従い一貫性を保ち、単体テストでバーンカードの正確な検証が可能になる。
