...
**Objective:** 開発者として、`getNextActivePlayer`を純粋関数として抽出し、単体テスト可能にしたい。これにより、プレイヤー遷移ロジックの正しさを独立して検証できるようにする。
...
**Objective:** 開発者として、`isRoundOver`を純粋関数として抽出し、単体テスト可能にしたい。これにより、ラウンド終了判定ロジックの正しさを独立して検証できるようにする。
...
**Objective:** 開発者として、`calculateBlinds`を純粋関数として抽出し、単体テスト可能にしたい。これにより、ポジション計算ロジックの正しさを独立して検証できるようにする。
...
**Objective:** 開発者として、`applyAction`を純粋関数として抽出し、単体テスト可能にしたい。これにより、各アクション（Fold/Call/Raise）の処理ロジックの正しさを独立して検証できるようにする。
...
**Objective:** 開発者として、`determineWinner`を純粋関数として抽出し、単体テスト可能にしたい。これにより、勝者判定ロジックの正しさを独立して検証できるようにする。
...
**Objective:** 開発者として、リファクタリング後も`useGameEngine`の外部振る舞いが変わらないことを保証したい。これにより、UIやゲームプレイに影響を与えないことを確認する。
...
**Objective:** 開発者として、抽出した各関数の単体テストを作成し、ロジックの正しさを検証可能にしたい。
...
**Objective:** 開発者として、リファクタリングにより既存のテストが壊れないことを保証したい。
